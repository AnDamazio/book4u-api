...
export class ExchangeController {
    ...
    async createExchangeCreditsWithBook(@Param("bookRequired") bookRequired: number, @Param('token') token: string) {
        try {
            ...
            if (Number(user.credits) >= Number(book.price)) {
                ...
                return {
                    ...
                    tradeId: await this.exchangeWithCreditServices.getIdFromExchange(await
                        this.exchangeWithCreditServices.createExchangeWithCredit(exchangeCreditWithBook)
                    ),
                }
            } else {
